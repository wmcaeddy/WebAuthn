...
- [ ] Task: Audit server-side `userId` generation and persistence logic.
    - [ ] Analyze `src/` for `userId` mapping and generation.
    - [ ] Analyze `_test/server.php` for session and storage handling of user IDs.
- [ ] Task: Audit client-side `userId` handling during registration and login.
    - [ ] Inspect `_test/modern_client.html` for `userId` parameter passing and encoding.
    - [ ] Inspect `_test/client.html` for legacy `userId` handling.
- [ ] Task: Develop a manual reproduction checklist for cross-platform failure to confirm the mismatch.
